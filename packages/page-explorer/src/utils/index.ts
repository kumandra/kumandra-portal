...
const formatterCurrency = (coin:number): Currency =>{
  ...
  if (coin == 0) return {money: '0', suffix: 'TKMD'};
  let k = 1000; 
  let currencyStr = ['PICO','NANO','MICRO','MILLI','TKMD','KILO','MILL','BILL']; 
  let i = 0; 
  for(let l=0;l<8;l++){
...
const formatterCurrencyStr = (coin:number): string =>{
  ...
  if (coin == 0) return "0 TKMD";
  let k = 1000; 
    let currencyStr = ['PICO','NANO','MICRO','MILLI','TKMD','KILO','MILL','BILL']; //convert
  let i = 0;
  for(let l=0;l<8;l++){
    if(coin / Math.pow(k, l) < 1){
      break;
